...
**LLM Agent 활용과 하네스 엔지니어링**
Claude Code, Perplexity, Codex 등을 실제 워크플로우에 통합하면서, 단순 코드 생성과 Agent를 효과적으로 활용하는 것은 전혀 다른 역량임을 깨달았습니다. 명확한 컨텍스트 전달, Chat/Code 역할 분리, 생성된 코드를 검증·통합하는 하네스 엔지니어링 감각이 실제 생산성을 결정한다는 것을 체감했습니다.

**실전 운용을 통한 버그 분석 경험**
설계 단계에서 발견하지 못한 버그들이 실제 Use Case에서 드러났습니다. 들여쓰기 한 줄 차이로 마지막 종목만 진입하는 버그, 그리드 서치 중 실시간 파라미터 오염, 모듈 레벨 실행으로 인한 모드 전환 불가 등을 직접 발견하고 해결하면서, 증상에서 원인을 역추적하는 디버깅 역량을 키웠습니다.
...
**AI 기반 파라미터 자동 최적화**
현재는 사람이 직접 파라미터 범위를 설정하는 User-Driven 방식입니다. 추후 당일 거래 데이터를 기반으로 AI가 최적 파라미터를 자동 도출하고 백테스트까지 실행하는 AI-Driven Optimization 구조로 발전시키는 것이 목표입니다.
